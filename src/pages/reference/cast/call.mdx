...
Options:
      ...
      --delegate
          ...
          The destination's runtime code is applied as a code override on the
          `--from` address and the call is then made to that address, so the
          destination's code runs in the caller's storage context, like an
          on-chain `delegatecall`.
          
          Note that the executed code observes `msg.sender` (and `tx.origin`)
          equal to the `--from` address itself, whereas in an on-chain
          `delegatecall` `msg.sender` is preserved from the delegating
          contract's own caller.
