...
- cast safe create $OWNER_1 $OWNER_2 $OWNER_3 --threshold 2 --rpc-url $RPC
--account
  deployer
...
```bash
$ cast safe create --help
```

```txt
Usage: cast safe create [OPTIONS] <OWNERS>...
...
Options:
      ...
      --salt-nonce <SALT_NONCE>
          CREATE2 salt nonce. Defaults to Safe Protocol Kit's chain-specific
          nonce
      ...
      --fallback-handler <FALLBACK_HANDLER>
          CompatibilityFallbackHandler address. Pass the zero address to disable
          it
          
          [default: 0xfd0732Dc9E303f09fCEf3a7388Ad10A83459Ec99]
