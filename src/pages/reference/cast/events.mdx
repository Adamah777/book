...
- cast events --address $TOKEN "Transfer(address indexed,address
indexed,uint256)"

A lone 32-byte positional value is treated as a transaction hash. Qualify a raw
topic with
...
```txt
Usage: cast events [OPTIONS] <--tx-hash <TX_HASH>|--address <ADDRESS>|--from-block <FROM_BLOCK>|--to-block <TO_BLOCK>|SIG_OR_TOPIC> [TOPICS_OR_ARGS]...
