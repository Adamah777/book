...
Test options:
  -j, --threads <THREADS>
      ...
      --invariant-frontier-dir <PATH>
          Directory for invariant branch frontier artifacts
          
          [env: FOUNDRY_INVARIANT_FRONTIER_DIR=]

      --invariant-frontier-limit <COUNT>
          Maximum number of invariant branch frontier records to write per
          campaign
          
          [env: FOUNDRY_INVARIANT_FRONTIER_LIMIT=]
      ...
      --symbolic-check-invariant-frontiers
          Check invariants from imported stateful fuzz frontier prefixes before
          flipping comparisons
          
          [env: FOUNDRY_SYMBOLIC_CHECK_INVARIANT_FRONTIERS=]
...
Compiler options:
  ...
  -D, --deny <LEVEL>
      ...
      --use <SOLC_VERSION>
          Specify the solc version, path, or executable name on `PATH` to build
          with.
          
          Valid values are in the format `x.y.z`, `solc:x.y.z`, `path/to/solc`,
          or `solc`.
...
Source attribution:
  Coverage follows compiler source maps. Inherited modifier code is reported
  under the
  source where the modifier is declared. Dependency sources are excluded by
  default;
